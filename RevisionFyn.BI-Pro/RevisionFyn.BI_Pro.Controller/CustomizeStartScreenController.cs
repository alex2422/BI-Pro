...
namespace RevisionFyn.BI_Pro.Controller
{
    public class CustomizeStartScreenController
    {
        ...
        public void LoadCompaniesToComboBox(ComboBox dropDownClient1, ComboBox dropDownClient2, ComboBox dropDownClient3)
        {
            ...
            {
                dropDownClient1.SelectedItem = listOfClients.Where(company => company.CompanyName == listOfData[0].Client).FirstOrDefault();
                dropDownClient2.SelectedItem = listOfClients.Where(company => company.CompanyName == listOfData[1].Client).FirstOrDefault();
                dropDownClient3.SelectedItem = listOfClients.Where(company => company.CompanyName == listOfData[2].Client).FirstOrDefault();
            }
